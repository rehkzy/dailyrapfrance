...
// sauf en mode strict (aucune tolérance de faute de frappe, réservé au réglage "Mode difficile").
export function isCloseMatch(guess: string, answer: string, strict = false): boolean {
...
export function checkGuess(guess: string, artistName: string, title: string, strict = false): boolean {
  return isCloseMatch(guess, artistName, strict) || isCloseMatch(guess, title, strict);
}
